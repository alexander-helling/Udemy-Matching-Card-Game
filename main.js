...
//set count to 0, previous target, and delay
var count = 0;
...
//loop through the array like ojbect containing 'selected class
for (i=0; i < selected.length; i++){
  selected[i].classList.add('match');
}
};

// Reset guess after two attempts
...
// Add event listenter to grid
grid.addEventListener('click', function(event){
  // Declare variable to target our clicked item
  var clicked = event.target;
  //Do not allow the grid section itselft to be selected;
  //only select div inside the grid || unmatched || unselected
  if (clicked.nodeName === 'SECTION' || clicked === previousTarget || clicked.parentNode.classList.contains('match')||clicked.parentNode.classList.contains('selected')) {
    return;
  }
  // we only wan to add 'selected class if the current count is less than 2
  if (count < 2) {
    count++;
